...
export default function Auth() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      base44.analytics.track({ eventName: "user_logged_in", properties: { role: account.role || 'user', is_premium: account.is_premium || false } });
  ...
  return (
    ...
    >
      {/* Mobile back header */}
      <div className="flex items-center px-4 py-2 sm:hidden" style={{ paddingTop: `calc(env(safe-area-inset-top) + 8px)` }}>
        <button
          onClick={() => window.location.href = '/'}
          className="flex items-center justify-center w-11 h-11 rounded-xl bg-white/80 backdrop-blur shadow text-slate-700"
        >
